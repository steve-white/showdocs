/*
 ...
 This is a C-based HTTP server that listens for GET requests
 ...
 */
 ...
 int handle_request(int client_sock, Config *config) {
     ...
     if (strcmp(path, "exit.html") == 0) {
         ...
         build_full_path(config->root_dir, "exit.html", full_path, sizeof(full_path));
         FILE *test_exit = fopen(full_path, "rb");
         if (test_exit) {
             fclose(test_exit);
             send_http_response(client_sock, "HTTP/1.1 200 OK", "exit.html", date_buffer, config->root_dir);
             log_message(LOG_INFO, "Served exit.html - Stopping web server");
         } else {
             log_message(LOG_WARN, "exit.html not found, but stopping anyway");
         }
         return 0;  // Signal to stop server
     }
 ...
 int main(int argc, char *argv[])
 {
     ...
     #ifdef _WIN32
     SOCKET server_sock = create_server_socket(&config);
     #else
     int server_sock = create_server_socket(&config);
     #endif
     ...
     while (should_run) 
     {
         ...
         should_run = handle_request(client_sock, &config);
         close(client_sock);
     }
